...
All maps have a hexagonal shape and are defined by their radius. \hyperref[fig:arena]{Figure \ref{fig:arena}} shows an example of a map with a radius of $5$. In order to make testing easy, the game also includes a map editor (see \hyperref[editor]{Editor user documentation}), as shown in \hyperref[fig:map-editor]{Figure \ref{fig:map-editor}}. Maps can be saved to a file and loaded back again at any time (see \hyperref[map-format]{the Map File Format section} for more details).
